...
class Rooms {
	...
	static async createRoom(room_number, room_type, price, number_of_occupants, availability_status, image_url, description) {
		try {
			...
			return {
				error: `an error has occurred ${e.message}`, data: null, message: 'could  not insert into rooms table'
			}
		}
	...
	static async updateRoom(room) {
		...
		try {
			...
			const updatedRoom = await sqlPool.query(`
          UPDATE rooms 
          SET
              room_number =$1,
              room_type = $2, 
              ...
              description = $7
         WHERE room_id = $8 
         RETURNING *;`, [room_number, room_type, price, availability_status, image_url, number_of_occupants, room_description, room_id]);

			if (!updatedRoom.rows.length || updatedRoom.rowCount === 0) {
			...
			return {
				error: `an error occurred ${e.message}`, data: null, message: 'could not update room in rooms table'
			}
		}
	...
	static async deleteRoomById(id) {
			...
			return {
				error: `an error occurred ${e.message}`, data: null, message: 'could not delete room in rooms table'
			}
		}
	}
	static async allRooms() {
		...
		} else return {error: null, data: rooms.rows, message: 'success'}
	...
	static async updateRoomById(id) {
		try {
			...
			if (updateRoom.rows.length === 0 || updateRoom.rowCount === 0) return {
